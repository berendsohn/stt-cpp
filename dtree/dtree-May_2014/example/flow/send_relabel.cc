...
struct NodeBase : public Element {
  NodeBase();
  Dart* last;
  Dart* current;
  int label;
...
NodeBase::NodeBase() : last(NULL), current(NULL), label(0), excess(0) {
}

typedef dtree::WithAncAggr<dtree::Min<int>,
                           dtree::WithAncValue<dtree::Add<int>,
                                               dtree::Begin<NodeBase> > > Cap;
typedef dtree::EndTree<Cap> Node;
...
inline void State::CutOne(Node* u) {
  if (Cut(u)) {
    Dart* a = u->current;
    int new_cap = Cap::Value(u);
    a->rev->cap += a->cap - new_cap;
    a->cap = new_cap;
  }
  Cap::SetValue(u, std::numeric_limits<int>::max());
}

inline void State::CutAll() {
  for (int i = 1; i <= n_; ++i) CutOne(&node_[i]);
}
...
inline void State::Discharge(Node* u) {
  ...
  do {
    ...
    u->current = a;
    Cap::SetValue(u, a->cap);
    Link(u, v);
    while (true) {
      Node* r = Root(u);
      if (r == u) break;
      int bottleneck = Cap::AggrAnc(u);
      bool no_excess = u->excess <= bottleneck;
      int delta = no_excess ? u->excess : bottleneck;
      u->excess -= delta;
      Cap::SubtractFromAnc(u, delta);
      if (r->excess == 0) Activate(r);
      r->excess += delta;
      while (Node* w = Cap::FindRootmostAnc(u, dtree::LessEqual(0))) CutOne(w);
      if (no_excess) {
        u->current = a;
        Deactivate(u);
        return;
  ...
  for (a = u[-1].last; a != u->last; ++a) {
    Node* v = a->head;
    if (v->current == a->rev) CutOne(v);
    if (a->cap > 0) low = std::min(low, v->label);
  }
  SetLabel(u, low + 1);
  // gap heuristic
