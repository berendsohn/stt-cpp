...
#ifndef DTREE_SELFADJUST_TREE_INL_H_
#define DTREE_SELFADJUST_TREE_INL_H_
...
class ScopedCutDashed {
 ...
 private:
  Node* excl_part_;
  Node* incl_part_;
  DISALLOW_COPY_AND_ASSIGN(ScopedCutDashed);
};
...
inline Type WithStaticValue<Type, Base>::Value(Node* node) {
  typedef WithValue<dtree::Nop<Type>, Base> Base1;
  assert(node);
...
inline void WithDescValue<Group, Base>::SetValue(Node* node,
                                                 typename Group::Type value) {
  typedef WithValue<dtree::WithIdentityFilter<Group>, Base> Base1;
...
#endif  // DTREE_SELFADJUST_TREE_INL_H_
